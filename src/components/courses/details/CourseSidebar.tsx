      ...
      <div className={`p-6 ${theme === 'dark' ? 'bg-gray-800/95' : 'bg-indigo-50/80'}`}>
        ...
        <div className={`space-y-4 mb-6 ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-white'} p-4 rounded-lg shadow-md`}>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className={`font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{course.createdBy || 'Անանուն'}</span>
          </div>
          {course.institution && (
            <div className="flex justify-between items-center">
              <span className={`flex items-center ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                <Award className={`h-4 w-4 mr-2 ${theme === 'dark' ? 'text-indigo-300' : 'text-indigo-500'}`} />
                Հաստատություն
              </span>
        ...
        <Button asChild variant="outline" className={`w-full rounded-full ${theme === 'dark' ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-indigo-300 text-indigo-600 hover:bg-indigo-50'}`}>
